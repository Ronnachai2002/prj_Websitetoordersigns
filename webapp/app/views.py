...
def register(req):
    if req.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            messages.error(req, "Username ถูกใช้ไปแล้ว.")
            return render(req, "app/register.html")

        if User.objects.filter(email=email).exists():
            messages.error(req, "Email ถูกใช้ไปแล้ว.")
            return render(req, "app/register.html")
...
def chat(request):
    ...
    if request.method == "POST":
        ...
        if admin:
            ...
            content = request.POST.get("content")
            image = request.FILES.get("image")
            message = Message(sender=sender, receiver=receiver, content=content, image=image)
            message.save()
    context = {
    }
...
def admin_chat(request, id):
    ...
    if request.method == "POST":
        ...
        image = request.FILES.get("image")
        message = Message(sender=user, receiver=sender, content=content, image=image)
        message.save()
    context = {
        'sender': sender,
    }
...
def admin_messages(request, id):
    ...
    for msg in chat_list:
        ...
        if msg.image:
            msg_data['image'] = request.build_absolute_uri(settings.MEDIA_URL + msg.image.name)
...
def payment_slip(request):
    if request.method == "POST":
        ...
        try:
            ...
            raise Http404("การชำระเงินที่คุณค้นหาไม่มีอยู่")
